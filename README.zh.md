...
```sh
dsh plugin --profile web add git+https://github.com/xavier711/dsh-deepseek-usage.git#v0.7.0
```

**没有全局安装过 `dsh`？** 用这条（npx 会自动下载）：

```sh
npx --yes @deepseek-ai/dsh plugin --profile web add git+https://github.com/xavier711/dsh-deepseek-usage.git#v0.7.0
```
...
**Git 安装的用户**：你安装的是固定 tag 的快照，**不会自动更新**——但你也不用自己去查：插件每次打开用量面板时（带小时级缓存）会查询 GitHub 最新发布，如果有新版本，面板顶部会显示**「发现新版本」**提示条。

点提示条里的**「立即更新」**，插件会直接帮你装上新版本，不用碰终端。装完面板会告诉你结果；如果界面看着没变，重启一下 App 即可。（安装动作交给 harness 的插件管理器执行：面板只是请它把本插件装到新 tag 上，不做别的事。）

有两种情况不显示按钮、只给命令：一是你用的 DSH 版本没有那个插件管理器，二是 `link:` 开发安装（重新安装会把你的开发目录换成 tag 副本）。无论哪种，命令就在提示条里：
