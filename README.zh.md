...
- **本地用量**：回放你本机会话日志里的官方 token 计数 —— 今日 / 近 7 天 / 累计、7 天柱状图、**12 个月活动图**（日/周/累计三种视图：日视图一天一格，悬停看当天完整构成；周视图每周一根柱；累计视图逐周累加）、今日构成、**按模型统计**、**按工作区统计**（工作区 = 会话所在的项目目录，折叠面板按工作区展开查看各自的会话记录；子代理会话计入其父工作区）
