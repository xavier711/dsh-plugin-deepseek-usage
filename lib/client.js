...
.du-activityMonths{flex:none;position:relative;height:12px;margin-left:34px;display:block}
...
.du-activityLabels span{color:var(--dsw-alias-label-caption);height:5px;text-align:center;font-size:9px;line-height:5px;flex:none}
.du-activityGrid{flex:none;gap:1px;display:flex}
.du-activityWeek{flex:none;flex-direction:column;gap:1px;display:flex}
/* 格子 5px + 1px 间距 = 每列 6px：面板固定 440px，卡片内容宽 390px，去掉 26px
   星期标签和 8px 间距只剩 356px，53 列（周一对齐时 12 个月最多 53 列）需要 317px。
   早先用 6px 格子（每列 7px → 53 列 370px）会超出 14px，被 .du-body 的
   overflow-x:hidden 悄悄裁掉最右边一两列 —— 探针里的 BUDGET 就是盯这个的。 */
...
/* 热力图配色：自带蓝色阶，深浅两套显式写死。不要换回 --dsw-static-deepseek-900/800
   —— 它们在深色面板（#1b1d21）上只差几个百分点，整块网格会看起来是空的。0 档优先用
   DSH 的 fill-l2，取不到时用自带灰（透明会让空格和面板背景连成一片）。 */
.du-activityCell[data-level="0"],.du-calSwatch[data-level="0"]{background:var(--dsw-alias-fill-l2,#e7ebf0)}
.du-activityCell[data-level="1"],.du-calSwatch[data-level="1"]{background:#dbeafe}
.du-activityCell[data-level="2"],.du-calSwatch[data-level="2"]{background:#93c5fd}
.du-activityCell[data-level="3"],.du-calSwatch[data-level="3"]{background:#3b82f6}
.du-activityCell[data-level="4"],.du-calSwatch[data-level="4"]{background:#1d4ed8}
body[data-ds-dark-theme] .du-activityCell[data-level="0"],body[data-ds-dark-theme] .du-calSwatch[data-level="0"]{background:var(--dsw-alias-fill-l2,#272a30)}
body[data-ds-dark-theme] .du-activityCell[data-level="1"],body[data-ds-dark-theme] .du-calSwatch[data-level="1"]{background:#1e3a8a}
body[data-ds-dark-theme] .du-activityCell[data-level="2"],body[data-ds-dark-theme] .du-calSwatch[data-level="2"]{background:#2563eb}
body[data-ds-dark-theme] .du-activityCell[data-level="3"],body[data-ds-dark-theme] .du-calSwatch[data-level="3"]{background:#3b82f6}
body[data-ds-dark-theme] .du-activityCell[data-level="4"],body[data-ds-dark-theme] .du-calSwatch[data-level="4"]{background:#60a5fa}
`;
...
/** Skeleton screen: shimmer placeholders matching the final card layout. */
    ...
    /**
     * Token 活动热力图：一列一周、一行一天（周一在最上），格子深浅 = 当天 token 量，
     * 悬停给出当天完整构成。柱状图看趋势，这张图看「断没断」—— 12 个月一眼扫完。
     */
    function TokenActivity({ data, t }) {
      ...
      // 分档按「有量的那些天」的**不同取值**排名再摊到 1~4 档：同一天的量必然同色，
      // 用量差几个数量级也能铺满整条色阶。若改用固定阈值，真实数据几乎总是全浅或全深；
      // 若按原始分位点切，重复值（同一天量相同）会把相邻档并掉、只剩两三个颜色。
      const amounts = days
      ...
      // 周一为一周之始（与本地日历一致地算星期）；数据首尾不足一周的位置补空格。
      ...
      // 月份标签只在该周跨进新月份时打一个；挨太近就跳过（6px 的格子放不下）。
      const months = [];
      let lastMonth = "";
      let lastColumn = -9;
      ...
      const renderWeek = (week, weekIndex) => react.createElement("div", { key: weekIndex, className: styles.activityWeek },
        week.map((day, row) => {
          ...
          const content = react.createElement(TipBody, {
            heading: `${day.date} · ${t(`local.wd${new Date(`${day.date}T00:00:00`).getDay()}`)}`,
            rows: tipRows(day, t)
          });
          return react.createElement("span", {
            key: row,
      ...
      return react.createElement("div", { className: styles.card },
        react.createElement("div", { className: styles.cardHead },
          react.createElement("div", { className: styles.cardTitle }, t("local.activity")),
          react.createElement("div", { className: styles.activitySubtitle }, t("local.last12Months"))
        ),
        react.createElement("div", { className: styles.activity },
          react.createElement("div", { className: styles.activityMonths },
            months.map((month) =>
              react.createElement("span", { key: month.key, className: styles.activityMonth, style: { left: `${month.index * 6}px` } }, month.label)
            )
          ),
          react.createElement("div", { className: styles.activityBody },
            react.createElement("div", { className: styles.activityLabels },
              [0, 1, 2, 3, 4, 5, 6].map((row) =>
                react.createElement("span", { key: row }, row % 2 === 0 && row < 6 ? t(`local.wd${(row + 1) % 7}`) : "")
              )
            ),
