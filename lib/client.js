...
/* 悬停整列：鼠标停在哪一周，那一列的 7 颗格子**一起**提亮（投影 + 同色柔光），
   其余列压暗一点，让这一列跳出来。用户要的就是这个 —— 「是 hover 的时候」「整列 hover，
   不是每个格子」。三条红线：不给列垫底色、不常驻每格阴影、不压间距合并成一列。 */
.du-activityGrid:hover .du-activityWeek:not(:hover) .du-activityCell{opacity:.35}
.du-activityWeek:hover .du-activityCell{opacity:1;box-shadow:0 1px 2px rgba(0,0,0,.18),0 0 5px rgba(34,197,94,.45)}
body[data-ds-dark-theme] .du-activityWeek:hover .du-activityCell{box-shadow:0 1px 3px rgba(0,0,0,.5),0 0 7px rgba(34,197,94,.55)}
