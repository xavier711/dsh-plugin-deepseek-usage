...
/** Skeleton screen: shimmer placeholders matching the final card layout. */
    ...
    const zh = {
      ...
      "config.loadingHint": "若一直停在这里：本表单需要宿主端提供 schema，而宿主端代码不会热更新（浏览器端 bundle 会）。请在运行 dsh web 的终端按 Ctrl+C 后重新启动，再刷新本页。",
      ...
      "config.reset": "恢复默认",
      "config.footer": "保存即写入 ~/.dsh/profiles/web/cordis.patch.yml 的 deepseek-usage 行；重启 dsh web 后生效（本机实测：纯配置改动不会被宿主热重载）。留空即恢复内置默认值，未列出的键仍可直接在 YAML 里配置。",
      ...
      "config.hint.pricing": "结构同 DEFAULTS.pricing，例如 {\"deepseek-flash\":{\"peak\":{\"input\":2}}}。",
    ...
    const en = {
      ...
      "config.loadingHint": "Stuck here? This form needs the host half, and host code is not hot-reloaded (the browser bundle is). Press Ctrl+C in the terminal running dsh web, start it again, then refresh this page.",
      ...
      "config.reset": "reset",
      "config.footer": "Saving writes the deepseek-usage row in ~/.dsh/profiles/web/cordis.patch.yml; restart dsh web to apply (a config-only change is not hot-reloaded by the host). Clearing a field restores the built-in default, and keys not listed here stay configurable in YAML.",
      ...
      "config.hint.pricing": "Same shape as DEFAULTS.pricing, e.g. {\"deepseek-flash\":{\"peak\":{\"input\":2}}}.",
