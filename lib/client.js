...
.du-activityCard{--du-heat-ink:#b4600f;--du-heat-bar:#d98b4a}
body[data-ds-dark-theme] .du-activityCard{--du-heat-ink:#feb484;--du-heat-bar:#feb484}
...
/* 参考 Codex 的 /usage：数值用暖色强调，和热力图的橙阶同一族 */
.du-activityCard .du-summaryValue{color:var(--du-heat-ink)}
...
.du-activityLabels{flex:none;flex-direction:column;gap:2px;width:18px;display:flex}
.du-activityLabels span{color:var(--dsw-alias-label-caption);height:5px;text-align:center;font-size:8px;line-height:5px;flex:none}
.du-activityGrid{flex:none;gap:2px;display:flex}
.du-activityWeek{flex:none;flex-direction:column;gap:2px;display:flex}
/* 格距是算出来的：面板固定 440px → 卡片内容 390px → 减去 18px 星期标签和 6px 间距，
   网格最多 366px。取最近 52 周（52 列），5px 格子 + 2px 间距 = 每列 7px → 362px。
   格子再大一点就会超出，被 .du-body 的 overflow-x:hidden 悄悄裁掉最右边几列，
   所以改尺寸必须看探针的 BUDGET=...:fit|OVERFLOW。 */
.du-activityCell{width:5px;height:5px;border-radius:1.5px;flex:none;position:relative;transition:opacity .1s}
...
.du-activityCell[data-level="0"],.du-calSwatch[data-level="0"]{background:var(--dsw-alias-fill-l2,#eceef0)}
.du-activityCell[data-level="1"],.du-calSwatch[data-level="1"]{background:#f6ddc7}
.du-activityCell[data-level="2"],.du-calSwatch[data-level="2"]{background:#e5b184}
.du-activityCell[data-level="3"],.du-calSwatch[data-level="3"]{background:#cf8646}
.du-activityCell[data-level="4"],.du-calSwatch[data-level="4"]{background:#a8541a}
body[data-ds-dark-theme] .du-activityCell[data-level="0"],body[data-ds-dark-theme] .du-calSwatch[data-level="0"]{background:var(--dsw-alias-fill-l2,#2c2f2e)}
body[data-ds-dark-theme] .du-activityCell[data-level="1"],body[data-ds-dark-theme] .du-calSwatch[data-level="1"]{background:#443329}
body[data-ds-dark-theme] .du-activityCell[data-level="2"],body[data-ds-dark-theme] .du-calSwatch[data-level="2"]{background:#745440}
body[data-ds-dark-theme] .du-activityCell[data-level="3"],body[data-ds-dark-theme] .du-calSwatch[data-level="3"]{background:#b27f5e}
body[data-ds-dark-theme] .du-activityCell[data-level="4"],body[data-ds-dark-theme] .du-calSwatch[data-level="4"]{background:#feb484}
`;
...
/** Skeleton screen: shimmer placeholders matching the final card layout. */
    ...
    function LocalContent({ local, t }) {
      ...
      return react.createElement(react.Fragment, null,
        ...
        react.createElement(TokenActivity, { data, t }),
    ...
    /**
     * Token 活动（对齐 Codex 的 /usage，配色取样自它的界面）：日视图是一天一格的
     * 热力图（周日在上、最近 52 周），周视图是每周一根柱，累计视图是逐周累加。
     * 标题右边切视图，下面一行是累计/单日峰值/连续天数。柱状图看走势，热力图看
     * 「断没断」。
     */
    function TokenActivity({ data, t }) {
      const [tip, show, hide] = useChartTip();
      ...
      // 分档按「有量的那些天」的不同取值排名再摊到 1~4 档：同一天的量必然同色，
      // 用量差几个数量级也能铺满整条色阶。用固定阈值会几乎总是全浅或全深；用原始
      // 分位点则会被重复值把相邻档并掉，只剩两三个颜色。
      const amounts = days
        .filter((day) => day.date <= todayKey)
        .map((day) => bucketTokens(day))
        .filter((tokens) => tokens > 0)
        .sort((a, b) => a - b);
      const distinct = [...new Set(amounts)];
      const rankOf = new Map(distinct.map((tokens, index) => [tokens, index]));
      const levelOf = (tokens) => {
        if (tokens <= 0 || distinct.length === 0) return 0;
        const index = rankOf.has(tokens) ? rankOf.get(tokens) : distinct.findIndex((value) => value >= tokens);
        const position = index < 0 ? distinct.length - 1 : index;
        return Math.min(4, 1 + Math.floor((position * 4) / distinct.length));
      };

      // 周日为一周之始（参考图第一行是 Su）；不足一周的位置补空。只保留最近 52 周，
      // 正好覆盖 12 个月，也让网格宽度可控（见 css 里的格距说明）。
      ...
      // 每周边界换算成一根柱子用的聚合桶（柱状图与累计图共用）。
      const weekBuckets = shown.map((week) => {
        const bucket = { date: "", calls: 0, inputTokens: 0, outputTokens: 0, cacheReadTokens: 0, cacheWriteTokens: 0, reasoningTokens: 0, cost: 0 };
        for (const day of week) {
          if (day === null || day.date > todayKey) continue;
          if (bucket.date === "") bucket.date = day.date;
      ...
      // 月份标签：该周跨进新月份时打一个，挨太近就跳过（格子只有 5px 宽）。
      const months = [];
      let lastMonth = "";
      let lastColumn = -9;
      ...
      const renderWeek = (week, weekIndex) => react.createElement("div", { key: weekIndex, className: styles.activityWeek },
        week.map((day, row) => {
          if (day === null || day.date > todayKey) {
            return react.createElement("span", { key: row, className: styles.activityCell, "data-level": "0", "data-dim": "" });
          }
          const content = react.createElement(TipBody, {
            heading: `${day.date} · ${t(`local.wd${weekdayIndex(day.date)}`)}`,
            rows: tipRows(day, t)
          });
          return react.createElement("span", {
            key: row,
            className: styles.activityCell,
            "data-level": `${levelOf(bucketTokens(day))}`,
      ...
      // 周视图 / 累计视图：单色柱子（同参考图），悬停给出该周的完整构成。
      const barW = 380;
      const barH = 120;
      const barBase = 96;
      const barTop = 10;
      ...
      const series = weekBuckets.map((bucket) => {
        if (!cumulative) return { bucket, value: bucketTokens(bucket) };
        running = running === null ? { ...bucket } : {
          date: running.date, calls: running.calls + bucket.calls,
          inputTokens: running.inputTokens + bucket.inputTokens,
          outputTokens: running.outputTokens + bucket.outputTokens,
          cacheReadTokens: running.cacheReadTokens + bucket.cacheReadTokens,
          cacheWriteTokens: running.cacheWriteTokens + bucket.cacheWriteTokens,
          reasoningTokens: running.reasoningTokens + bucket.reasoningTokens,
          cost: running.cost + bucket.cost
        };
        return { bucket: running, value: bucketTokens(running) };
      });
      const peak = Math.max(1, ...series.map((point) => point.value));
      const slot = (barW - padLeft) / Math.max(1, series.length);
      const width = Math.max(2, Math.min(10, slot - 2));
      const bars = series.map((point, index) => {
        const height = (point.value / peak) * (barBase - barTop);
        const x = padLeft + index * slot + (slot - width) / 2;
        const content = react.createElement(TipBody, {
          heading: point.bucket.date === "" ? t("local.activity") : point.bucket.date,
          rows: tipRows(point.bucket, t)
        });
        return react.createElement("g", { key: index, className: "du-barGroup", onMouseEnter: (event) => show(event, content), onMouseLeave: hide },
          react.createElement("rect", {
            x, y: barBase - height, width, height: Math.max(point.value > 0 ? 2 : 0, height), rx: 1.5,
            style: { fill: "var(--du-heat-bar)" }
          })
        );
      });
      const axis = react.createElement(react.Fragment, null,
        react.createElement("text", { x: 0, y: barTop + 7, style: { fill: "var(--dsw-alias-label-caption)", fontSize: 9 } }, "max"),
        react.createElement("text", { x: 0, y: barBase, style: { fill: "var(--dsw-alias-label-caption)", fontSize: 9 } }, "0"),
        react.createElement("line", { x1: padLeft, y1: barBase, x2: barW, y2: barBase, style: { stroke: "var(--dsw-alias-border-l2)", strokeWidth: 1 } }),
        months.map((month) =>
          react.createElement("text", {
            key: month.key, x: padLeft + month.index * slot, y: barH - 2,
            style: { fill: "var(--dsw-alias-label-caption)", fontSize: 9 }
          }, month.label)
        )
      );
      ...
      const caption = cumulative
        ? `${t("local.cumulativeCaption")} ${fmtCompact(peak)}`
        : `${t("local.weeklyCaption")} ${fmtCompact(peak)}`;

      return react.createElement("div", { className: `${styles.card} ${styles.activityCard}` },
        react.createElement("div", { className: styles.cardHead },
          react.createElement("div", { className: styles.cardTitle },
            t("local.activity"),
            react.createElement("span", { className: styles.activitySubtitle }, t("local.last12Months"))
          ),
        ...
        view === "daily"
          ? react.createElement("div", { className: styles.activity },
              react.createElement("div", { className: styles.activityMonths },
                months.map((month) =>
                  react.createElement("span", { key: month.key, className: styles.activityMonth, style: { left: `${month.index * 7}px` } }, month.label)
                )
              ),
              react.createElement("div", { className: styles.activityBody },
                react.createElement("div", { className: styles.activityLabels },
                  [0, 1, 2, 3, 4, 5, 6].map((row) =>
                    react.createElement("span", { key: row }, t(`local.wd${row}`))
                  )
                ),
                react.createElement("div", { className: styles.activityGrid, role: "img", "aria-label": t("local.activity") },
          ...
          : react.createElement("div", { className: styles.activity },
              react.createElement("svg", { viewBox: `0 0 ${barW} ${barH}`, className: `${styles.chart} ${styles.bars}`, role: "img", "aria-label": t("local.activity") },
                axis,
                bars
              ),
              react.createElement("div", { className: styles.activityCaption }, caption)
            ),
        react.createElement(ChartTip, { tip })
      );
    }
