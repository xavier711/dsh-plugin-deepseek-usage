...
.du-activityCell[data-level="0"],.du-calSwatch[data-level="0"]{background:var(--dsw-alias-fill-l2,#edf3fe)}
.du-activityCell[data-level="1"],.du-calSwatch[data-level="1"]{background:#b7c8fe}
.du-activityCell[data-level="2"],.du-calSwatch[data-level="2"]{background:#7aaaff}
.du-activityCell[data-level="3"],.du-calSwatch[data-level="3"]{background:#4176e6}
.du-activityCell[data-level="4"],.du-calSwatch[data-level="4"]{background:#34415b}
body[data-ds-dark-theme] .du-activityCell[data-level="0"],body[data-ds-dark-theme] .du-calSwatch[data-level="0"]{background:var(--dsw-alias-fill-l2,#283142)}
body[data-ds-dark-theme] .du-activityCell[data-level="1"],body[data-ds-dark-theme] .du-calSwatch[data-level="1"]{background:#34415b}
body[data-ds-dark-theme] .du-activityCell[data-level="2"],body[data-ds-dark-theme] .du-calSwatch[data-level="2"]{background:#4868b2}
body[data-ds-dark-theme] .du-activityCell[data-level="3"],body[data-ds-dark-theme] .du-calSwatch[data-level="3"]{background:#5686fe}
body[data-ds-dark-theme] .du-activityCell[data-level="4"],body[data-ds-dark-theme] .du-calSwatch[data-level="4"]{background:#7aaaff}
...
@keyframes du-cell-in{from{transform:scale(.35)}to{transform:scale(1)}}
.du-bars rect{transform-box:fill-box;transform-origin:bottom;animation:du-bar-grow .5s cubic-bezier(.22,.8,.3,1) both}
.du-activityCell{animation:du-cell-in .3s ease-out both}
@media (prefers-reduced-motion: reduce){
  .du-bars rect,.du-activityCell{animation:none;transform:none}
}
`;
...
/** Skeleton screen: shimmer placeholders matching the final card layout. */
    ...
    /**
     * Token 活动：一天一格的热力图（周日在上，取最近 6 个月），格子深浅 = 当天 token
     * 量，悬停给出当天完整构成。和上面那张趋势卡各管一件事 —— 趋势卡看走势与构成，
     * 这张看「有没有断」。只画 6 个月是因为 440px 的侧边栏塞 12 个月时格子只剩 5px，
     * 会糊成一片网（见 css 里的格距说明）。
     */
    function TokenActivity({ data, t, anim }) {
      const [tip, show, hide] = useChartTip();
      // 这张卡通常在折叠线以下：等滚到它时挂载动画早演完了。所以除了刷新（anim 变化），
      // 每次它进入视口也重放一次；重放靠给网格换 key 触发重新挂载。
      const [play, setPlay] = react.useState(0);
      const cardRef = react.useRef(null);
      react.useEffect(() => { setPlay((current) => current + 1); }, [anim]);
      react.useEffect(() => {
        const node = cardRef.current;
        if (node === null || typeof IntersectionObserver !== "function") return undefined;
        const observer = new IntersectionObserver((entries) => {
          for (const entry of entries) if (entry.isIntersecting) setPlay((current) => current + 1);
        }, { threshold: 0.25 });
      ...
      // 周日为一周之始；不足一周的位置补空。取最近 26 周（6 个月）。
      ...
      // 逐列错峰出现（每列 16ms，26 列共 ~0.4s）；减少动态效果时 css 里整段关掉。
