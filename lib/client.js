...
.du-periodTag{box-sizing:border-box;flex:none;align-items:center;gap:4px;height:18px;border-radius:9px;padding:0 6px;font-size:10px;line-height:18px;display:inline-flex}
...
.du-sessionWs{color:var(--dsw-alias-label-tertiary);background:var(--dsw-alias-fill-l2);border-radius:6px;flex:none;max-width:110px;text-overflow:ellipsis;white-space:nowrap;padding:0 5px;font-size:10px;line-height:15px;overflow:hidden}
.du-workspaceHead{box-sizing:border-box;flex:none;align-items:center;gap:8px;min-width:0;width:100%;color:inherit;cursor:pointer;background:0 0;border:none;padding:0;font-family:inherit;text-align:left;display:flex}
.du-workspaceHead:hover .du-workspaceName{color:var(--dsw-alias-state-business-primary)}
.du-workspaceName{min-width:0;color:var(--dsw-alias-label-primary);text-overflow:ellipsis;white-space:nowrap;flex:1;font-size:12px;font-weight:500;line-height:18px;overflow:hidden}
.du-workspaceMeta{color:var(--dsw-alias-label-tertiary);flex:none;font-size:11px;line-height:18px}
.du-workspacePath{color:var(--dsw-alias-label-caption);text-overflow:ellipsis;white-space:nowrap;font-family:var(--dsh-font-mono,monospace);font-size:10px;line-height:14px;overflow:hidden}
...
`;
    ...
    /** Trailing path segment (workspace display name), POSIX or Windows separators. */
    function pathBase(p) {
      const parts = p.split(/[\\/]/);
     ...
     * cost-scaled bars like the model list; each row expands to its own
     * session records. Hover shows the full token/cost breakdown.
     */
    function WorkspaceStats({ workspaces, t }) {
      const [tip, show, hide] = useChartTip();
      const [expanded, setExpanded] = react.useState(null);
      const maxCost = Math.max(1e-6, ...workspaces.map((w) => w.buckets.total.cost));
      return react.createElement("div", { className: styles.card },
        ...
        workspaces.map((w) => {
          ...
          const content = react.createElement(TipBody, { heading: name, rows: tipRows(total, t) });
          const open = expanded === w.path;
          return react.createElement("div", { className: styles.modelRow, key: w.path ?? "", onMouseEnter: (e) => show(e, content), onMouseLeave: hide },
            react.createElement("button", {
              ...
              react.createElement("span", { className: styles.workspaceName, title: w.path ?? undefined }, name),
              w.subagentSessionCount > 0
                ...
                : null,
              react.createElement("span", { className: styles.workspaceMeta },
                t("local.workspaceMeta", { tokens: fmtCompact(bucketTokens(total)), sessions: w.sessionCount, cost: fmtCost(total.cost) })
              )
            ),
            w.path !== null
              ? react.createElement("div", { className: styles.workspacePath, title: w.path }, w.path)
              : null,
            react.createElement("div", { className: styles.modelBar },
              react.createElement("span", { style: { width: `${(total.cost / maxCost) * 100}%` } })
            ),
            open ? react.createElement(SessionList, { sessions: w.sessions, t }) : null
          );
        }),
        react.createElement(ChartTip, { tip })
      );
    ...
    function SessionList({ sessions, t, showWorkspace }) {
        ...
        sorted.map((session) => {
          ...
          return react.createElement("li", { className: styles.sessionRow, key: session.id },
            react.createElement("div", { className: styles.sessionHead },
              ...
              showWorkspace && typeof session.workspace === "string"
                ? react.createElement("span", { className: styles.sessionWs, title: session.workspace }, pathBase(session.workspace))
                : null,
...
/** Skeleton screen: shimmer placeholders matching the final card layout. */
    function SkeletonScreen({ t }) {
      ...
      return react.createElement("div", { className: styles.skel, role: "status", "aria-label": t("loading") },
        ...
        react.createElement("div", { className: styles.skelCard },
          line("30%", 12),
          [0, 1, 2].map((i) => row(line("55%", 12), line("35%", 10)))
        )
      );
    }
    ...
    function LocalContent({ local, t }) {
      ...
      return react.createElement(react.Fragment, null,
        ...
        showSessions ? react.createElement(SessionList, { sessions: data.sessions, t, showWorkspace: true }) : null
      );
    }
    ...
    const zh = {
      ...
      "local.workspaceMeta": "{tokens} tok · {sessions} 个会话 · {cost}",
    ...
    const en = {
      ...
      "local.workspaceMeta": "{tokens} tok · {sessions} sessions · {cost}",
