...
/* 悬停整列 —— **只在周/累计视图生效**：这两个视图的单位是「一周」，鼠标停在哪一周就把那
   一列的 7 颗格子一起提亮（投影 + 同色柔光），其余列压暗。日视图一天一格，单位是格子而不是
   列，所以不参与（用户原话：「每日的你怎么也列高亮了」）。
   三条红线：不给列垫底色、不常驻每格阴影、不压间距合并成一列。
   ⚠️ **柔光半径要明显小于间距的一半**：格子 10px、间距 4px，原来写 5~7px 会跟上下左右邻格
   糊成一片（用户原话「高光范围太大了，都互相叠加了」）。现在投影 1px、柔光 2px。 */
.du-activityGrid:not([data-view="daily"]):hover .du-activityWeek:not(:hover) .du-activityCell{opacity:.35}
.du-activityGrid:not([data-view="daily"]) .du-activityWeek:hover .du-activityCell{opacity:1;box-shadow:0 1px 1px rgba(0,0,0,.16),0 0 2px rgba(34,197,94,.5)}
body[data-ds-dark-theme] .du-activityGrid:not([data-view="daily"]) .du-activityWeek:hover .du-activityCell{box-shadow:0 1px 1px rgba(0,0,0,.5),0 0 2px rgba(34,197,94,.6)}
