...
/** Skeleton screen: shimmer placeholders matching the final card layout. */
    ...
    /**
     * Token 活动：日视图是一天一格的热力图（周一在最上、最近 26 周 = 6 个月），周视图每周
     * 一根柱子、累计视图逐周累加（这两个是最近 12 个月），右上角切换 —— 和 Codex 的
     * /usage 一致。热力图只画 6 个月：440px 的侧边栏塞 12 个月时格子只剩 5px，会糊成
     * 一片网（见 css 里的格距说明）。汇总行（累计/峰值/连续天数）归「用量趋势」那张卡，
     * 这张不重复放。
     */
    function TokenActivity({ data, t, anim }) {
      ...
      // 周一为一周之始（用户在意的点：第一行必须是周一，不是周日）；不足一周的位置补空。
      ...
      const dailyWeeks = shown.slice(-26);

      // 每周聚合成一根柱子用的桶（周视图与累计视图共用）。
      ...
      // 周视图 / 累计视图：单色柱子（同参考图），悬停给出该周（或累计到该周）的完整构成。
      const barW = 380;
      const barH = 120;
      const barBase = 96;
      const barTop = 10;
      ...
      const bars = series.map((point, index) => {
        ...
        return react.createElement("g", { key: index, className: "du-barGroup", onMouseEnter: (event) => show(event, content), onMouseLeave: hide },
          react.createElement("rect", {
            ...
            style: { fill: "var(--dsw-alias-state-success-primary)", animationDelay: `${index * 8}ms` }
          })
        );
      });
        ...
        view === "daily"
          ? react.createElement("div", { className: styles.activity },
              ...
              react.createElement("div", { className: styles.activityBody },
                ...
                react.createElement("div", { key: `heat-${anim}-${play}`, className: styles.activityGrid, role: "img", "aria-label": t("local.activity") },
          ...
          : react.createElement("div", { className: styles.activity },
              react.createElement("svg", { key: `bars-${anim}`, viewBox: `0 0 ${barW} ${barH}`, className: `${styles.chart} ${styles.bars}`, role: "img", "aria-label": t("local.activity") },
                axis,
                bars
              ),
