 ...
 * The sidebar cost badge refreshes from activity signals (session snapshot
 * changes, tab focus/visibility, panel open) — no polling. Charts are
...
`;
     ...
     * 12:00 后转 空闲" (or the English equivalent). Falls back to a shorter
     * form when the next boundary is missing or already past.
     */
    function periodText(data, t) {
      ...
      if (typeof data.nextAt === "number" && data.nextAt > now + 1000) {
        const nextName = t(data.period === "peak" ? "period.offPeak" : "period.peak");
