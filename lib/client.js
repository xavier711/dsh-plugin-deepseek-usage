...
.du-updateLink{color:var(--dsw-alias-state-business-primary);flex:none;font-size:11px;line-height:16px;margin-left:auto}
.du-updateActions{flex:none;align-items:center;gap:6px;display:flex}
.du-updateButton,.du-updateButtonGhost{border-radius:6px;cursor:pointer;font-size:11px;line-height:16px;padding:3px 8px;border:1px solid transparent}
.du-updateButton{background:var(--dsw-alias-state-business-primary);color:#fff}
.du-updateButton:disabled{cursor:default;opacity:.6}
.du-updateButtonGhost{background:transparent;border-color:var(--dsw-alias-border-l2);color:var(--dsw-alias-label-secondary)}
.du-updateHint{color:var(--dsw-alias-label-tertiary);font-size:11px;line-height:16px}
.du-updateOk{color:var(--dsw-alias-state-success-primary);font-size:11px;line-height:16px}
.du-updateError{color:var(--dsw-alias-state-error-primary,#e5484d);font-size:11px;line-height:16px}
...
`;
    ...
    const styles = {
      ...
      updateActions: "du-updateActions",
      updateButton: "du-updateButton",
      updateButtonGhost: "du-updateButtonGhost",
      updateHint: "du-updateHint",
      updateOk: "du-updateOk",
      updateError: "du-updateError",
      note: "du-note",
      tip: "du-tip",
    ...
    /**
     * Update banner. Offers a one-click install when the host can do it
     * (`canInstall`), and always shows the copyable command as the fallback —
     * older harnesses have no plugin manager, and a `link:` development copy
     * must never be replaced by a tagged install.
     */
    function UpdateBanner({ update, t, onDone }) {
      const [phase, setPhase] = react.useState("idle"); // idle | running | done | failed
      const [message, setMessage] = react.useState(null);
      const [copied, setCopied] = react.useState(false);
      const available = update !== null && update.state === "ok" && update.data.updateAvailable === true;
      // 装完之后 installed 就等于 latest 了，`updateAvailable` 会变 false —— 但「已更新到 x，
      // 界面没变就重启」这句提示必须让用户看到，所以完成态自己撑住横幅，不跟着消失。
      if (!available && phase !== "done") return null;
      const latest = update?.data?.latest ?? null;
      const spec = typeof update?.data?.spec === "string" && update.data.spec.length > 0
        ? update.data.spec
        : `git+https://github.com/xavier711/dsh-deepseek-usage.git#v${latest}`;
      // link 安装永远不给一键更新按钮：宿主已经这么判了，这里再挡一道，避免两边不一致时
      // 把作者的开发目录换成 tag 副本。
      const linked = update?.data?.installKind === "link";
      const canInstall = available && !linked && update.data.canInstall === true;

      // Installing replaces this plugin on disk; the host reloads it, which can
      // drop the request that started it. So poll the version route until the
      // host reports the new version (or give up and tell the user to restart).
      const waitForInstalled = async () => {
        for (let attempt = 0; attempt < 40; attempt += 1) {
          await new Promise((resolve) => setTimeout(resolve, 1500));
          try {
            const response = await fetch("/dsh-usage/version", { headers: { Accept: "application/json" } });
            const json = await response.json();
            if (json?.ok === true && json.installed === latest) return true;
          } catch {
            /* the host is mid-reload — keep polling */
          }
        }
        return false;
      };
      ...
      const install = async () => {
        ...
        setMessage(null);
        try {
          const response = await fetch("/dsh-usage/update", { method: "POST", headers: { Accept: "application/json" } });
          const json = await response.json().catch(() => null);
          if (json?.ok !== true) {
            setPhase("failed");
            setMessage(typeof json?.message === "string" && json.message.length > 0 ? json.message : t("update.retry"));
            return;
          }
          const settled = await waitForInstalled();
          setPhase("done");
          setMessage(settled ? null : t("update.restart"));
      ...
      const copy = async () => {
        try {
          await navigator.clipboard.writeText(`dsh plugin --profile web add ${spec}`);
          setCopied(true);
          setTimeout(() => setCopied(false), 2000);
        } catch {
          /* clipboard blocked (no permission / insecure context) — the code line stays selectable */
        }
      };

      if (phase === "done") {
        return react.createElement("div", { className: styles.updateBanner },
          react.createElement("div", { className: styles.updateOk }, message === null ? t("update.done", { version: latest }) : message)
        );
      }

      const hint = update.data.installKind === "link"
        ? t("update.manualLinked")
        : canInstall ? null : t("update.manualHost");
      ...
      return react.createElement("div", { className: styles.updateBanner },
        ...
        react.createElement("code", { className: styles.updateCommand }, `dsh plugin --profile web add ${spec}`),
        react.createElement("div", { className: styles.updateActions },
          canInstall && phase !== "done"
            ? react.createElement("button", {
                type: "button",
                className: styles.updateButton,
                disabled: phase === "running",
                onClick: install
              }, phase === "running" ? t("update.running") : t("update.install"))
            : null,
          react.createElement("button", {
            type: "button",
            className: styles.updateButtonGhost,
            onClick: copy
        ...
        hint === null ? null : react.createElement("div", { className: styles.updateHint }, hint),
        phase === "failed" ? react.createElement("div", { className: styles.updateError }, t("update.failed", { message })) : null
      );
    }
...
/** Skeleton screen: shimmer placeholders matching the final card layout. */
      ...
      return react.createElement("div", { ref: rootRef, className: styles.root },
        ...
        open ? react.createElement("div", { className: styles.panel, role: "dialog", "aria-label": t("panel.title") },
          ...
          react.createElement("div", { className: styles.body },
            react.createElement(UpdateBanner, { update, t, onDone: refresh }),
    ...
    const zh = {
      ...
      "update.copied": "已复制",
      "update.running": "正在更新…",
      "update.done": "已更新到 v{version}。如果界面没变，重启一下 App。",
      "update.failed": "更新失败：{message}",
      "update.retry": "可以稍后重试，或复制下面的命令手动更新。",
      "update.restart": "已经装上新版本，重启一下 App 就会生效。",
      "update.manualLinked": "这是开发安装（link），请在开发目录里自行更新。",
      "update.manualHost": "当前 DSH 版本不支持面板内更新，用下面的命令即可。",
    ...
    const en = {
      ...
      "update.done": "Updated to v{version}. Restart the app if nothing changes.",
      "update.failed": "Update failed: {message}",
      "update.retry": "Try again later, or copy the command below.",
      "update.restart": "The new version is installed — restart the app to load it.",
      "update.manualLinked": "This is a development (link) install — update it from your checkout.",
      "update.manualHost": "This DSH build cannot update from the panel; use the command below.",
