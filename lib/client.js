...
/** Skeleton screen: shimmer placeholders matching the final card layout. */
    ...
    const zh = {
      ...
      "config.save": "保存",
      "config.saving": "保存中…",
      "config.saved": "已写入 profile 配置。",
      "config.refused": "宿主拒绝了这次写入（配置可能已被其它窗口改动），请重试。",
      "config.invalid": "{field} 的取值不合法。",
      "config.invalidShort": "取值不合法",
      "config.overridden": "已覆盖",
      "config.reset": "恢复默认",
      "config.footer": "保存后写入 ~/.dsh/profiles/web/cordis.patch.yml 的 deepseek-usage 行，宿主会即时重载；留空即恢复内置默认值。未列出的键仍可直接在 YAML 里配置。",
    ...
    const en = {
      ...
      "config.saved": "Written to the profile configuration.",
      ...
      "config.reset": "reset",
      "config.footer": "Saving writes the deepseek-usage row in ~/.dsh/profiles/web/cordis.patch.yml and the host reloads it; clearing a field restores the built-in default. Keys not listed here stay configurable in YAML.",
