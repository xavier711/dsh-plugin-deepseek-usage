...
.du-activityCell{width:10px;height:10px;border-radius:3px;flex:none;position:relative;transition:opacity .1s}
...
/** Skeleton screen: shimmer placeholders matching the final card layout. */
    ...
    function TokenActivity({ data, t }) {
      ...
      // 日视图只画最近 6 个月：格子在 440px 侧边栏里才能大到看得清（见 css 注释）。
      const DAILY_WEEKS = 26;
      const dailyWeeks = shown.slice(-DAILY_WEEKS);
      ...
      const monthLabels = (list) => {
        ...
        list.forEach((week, index) => {
          ...
          lastColumn = index;
          out.push({ key: month, index, label: t(`local.month${parseInt(month.slice(5, 7), 10)}`) });
        });
        return out;
      };
      const months = monthLabels(shown);
      const dailyMonths = monthLabels(dailyWeeks);
      ...
      return react.createElement("div", { className: `${styles.card} ${styles.activityCard}` },
        react.createElement("div", { className: styles.cardHead },
          react.createElement("div", { className: styles.cardTitle },
            t("local.activity"),
            react.createElement("span", { className: styles.activitySubtitle }, view === "daily" ? t("local.last6Months") : t("local.last12Months"))
          ),
