...
.du-sessionBar{background:var(--dsw-alias-fill-l2);border-radius:999px;flex:none;height:4px;display:flex;overflow:hidden}
.du-sessionBar>span{height:100%;display:block}
...
.du-wsCount{background:var(--dsw-alias-interactive-bg-hover);color:var(--dsw-alias-label-tertiary);font-variant-numeric:tabular-nums;border-radius:8px;flex:none;padding:0 6px;font-size:10px;line-height:16px}
...
`;
    ...
    /**
     * Per-session rows, most recently active first. The thin bar is a token
     * COMPOSITION stack (segment colors match the trend chart), scaled
     * against the largest session in the same list; hovering a row shows the
     * full breakdown (tokens by category + estimated cost).
     */
    function SessionList({ sessions, t }) {
      ...
      const segments = [
        ["cacheReadTokens", "var(--du-cacheRead)"],
        ["inputTokens", "var(--du-input)"],
        ["outputTokens", "var(--du-output)"],
        ["reasoningTokens", "var(--du-reasoning)"],
        ["cacheWriteTokens", "var(--du-cacheWrite)"]
      ];
        ...
        sorted.map((session) => {
          ...
          return react.createElement("li", {
            ...
            react.createElement("div", { className: styles.sessionBar },
              segments.map(([field, color]) => {
                const width = ((session[field] ?? 0) / max) * 100;
                if (width <= 0) return null;
                return react.createElement("span", { key: field, style: { width: `${width}%`, background: color } });
              })
            )
          );
        }),
