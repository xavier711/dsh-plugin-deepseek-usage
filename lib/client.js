...
/* 有量的格子加一道顶部内高光 + 轻外发光（"上釉"的观感，参考 Codex 那种一颗颗亮格子）；
   空格子保持纯色，不抢视线。注意别动列内间距、别把整列并成一块 —— 用户明确说过合并很丑。 */
.du-activityCell[data-level="1"],.du-activityCell[data-level="2"],.du-activityCell[data-level="3"],.du-activityCell[data-level="4"]{box-shadow:inset 0 1px 0 rgba(255,255,255,.16)}
.du-activityCell[data-level="4"]{box-shadow:inset 0 1px 0 rgba(255,255,255,.26),0 0 5px rgba(34,197,94,.4)}
body[data-ds-dark-theme] .du-activityCell[data-level="1"],body[data-ds-dark-theme] .du-activityCell[data-level="2"],body[data-ds-dark-theme] .du-activityCell[data-level="3"],body[data-ds-dark-theme] .du-activityCell[data-level="4"]{box-shadow:inset 0 1px 0 rgba(255,255,255,.28)}
body[data-ds-dark-theme] .du-activityCell[data-level="4"]{box-shadow:inset 0 1px 0 rgba(255,255,255,.34),0 0 6px rgba(34,197,94,.45)}
/* 鼠标所在的那一列整体轻微提亮（列背后垫一层淡淡的底），让人看出"这一周是一组" */
.du-activityWeek{position:relative}
.du-activityWeek[data-active]::before{content:"";position:absolute;inset:-3px -3px;border-radius:5px;background:var(--dsw-alias-fill-l2,#2a2e34);opacity:.55}
.du-activityWeek > *{position:relative}
...
/** Skeleton screen: shimmer placeholders matching the final card layout. */
    ...
    function TokenActivity({ data, t, anim }) {
      ...
      // 鼠标停在哪一列（哪一周）上：整列轻微提亮，让人看出这一周是一组。
      ...
      const valueOf = (day, week) => {
        ...
        if (view === "cumulative") return weekCumulative.get(week) ?? 0;
      ...
      const renderWeek = (week, weekIndex) => react.createElement("div", {
        key: weekIndex,
        className: styles.activityWeek,
        "data-active": weekIndex === hoverCol ? "" : undefined,
        onMouseEnter: () => setHoverCol(weekIndex),
        ...
        week.map((day, row) => {
          ...
          const content = react.createElement(TipBody, {
            heading: view === "daily"
              ? `${day.date} · ${t(`local.wd${(weekdayIndex(day.date) + 1) % 7}`)}`
              : `${week.find((item) => item !== null).date} · ${t(view === "weekly" ? "local.viewWeekly" : "local.viewCumulative")}`,
            rows
          });
          return react.createElement("span", {
            key: row,
      ...
      return react.createElement("div", { className: styles.card, ref: cardRef },
        ...
        react.createElement("div", { className: styles.activity },
          ...
          react.createElement("div", { className: styles.activityBody },
            ...
            react.createElement("div", { key: `heat-${view}-${anim}-${play}`, className: styles.activityGrid, role: "img", "aria-label": t("local.activity") },
