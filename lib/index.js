 ...
 * webserver (`/dsh-usage/balance`, `/dsh-usage/local`); the browser half
 * (`./client`) fetches them and renders the panel.
 *
...
function resolveRates(pricing, model, time, cfg) {
  ...
  if (typeof time !== "number" || time < cfg.newPricingAt || typeof entry?.peak !== "object" || typeof entry?.offPeak !== "object") {
    return { ...flat, estimated: !known };
  }
  const beijingMinutes = (time + cfg.timezoneOffsetMinutes * 60_000) % 86_400_000;
  const hour = Math.floor(beijingMinutes / 3_600_000);
  const isPeak = cfg.peakHours.some(([start, end]) => hour >= start && hour < end);
  const rates = isPeak ? entry.peak : entry.offPeak;
  return {
...
function apply(ctx, config = {}) {
  ...
  const disposers = [
    ...
    ctx.webServer.register({
      ...
      handler: async (_req, res) => {
        try {
          sendJson(res, 200, await computeLocalUsage(ctx, cfg));
        } catch (error) {
          sendJson(res, 500, { ok: false, error: "INTERNAL", message: String(error?.message ?? error) });
        }
      }
