...
/**
 * Unwrap the `Volatile` holders cordis hands a plugin whose `Config` schema
 * declares the key.
 *
 * `resolveConfig()` validates through the schema but returns an *unresolved*
 * shape: every declared key is a holder whose single own property is `get`,
 * and `get()` yields the effective value (the user's, or the schema default).
 * JSON-serializing a holder looks like `{}`, which is why an unresolved config
 * can be mistaken for "the user set nothing" — taking the holders at face
 * value silently ignores the row config entirely.
 *
 * @param config - the config as the loader validated it.
 * @returns the same keys with plain values; holders whose `get()` throws are
 * dropped so {@link DEFAULTS} stays in force for them.
 */
function unwrapConfig(config) {
  ...
  for (const [key, value] of Object.entries(config)) {
    ...
    try {
      ...
      /* leave the key out: DEFAULTS wins */
    }
  }
  return plain;
}
...
/**
 * Merge row config over {@link DEFAULTS}, ignoring the placeholders cordis
 * leaves behind for keys the user did not set.
 *
 * With an exported `Config` schema, the value handed to `apply()` has been
 * through `resolveConfig()`, which validates but does **not** resolve defaults:
 * every unset key arrives as `{}` (`schemastery` materializes defaults later,
 * for the settings form). A plain spread would then let `maxSessions: {}`
 * overwrite the real default, so a value is only taken when it is usable.
 *
 * @param defaults - the built-in defaults (the single source of truth).
 * @param config - the row config as the loader validated it.
 * @returns the effective config.
 */
function mergeConfig(defaults, config) {
  const merged = { ...defaults };
  if (typeof config !== "object" || config === null) return merged;
  for (const [key, value] of Object.entries(config)) {
    if (value === undefined) continue;
    // An explicit null is meaningful here (it turns a URL-backed feature off).
    if (value === null) {
      merged[key] = null;
      continue;
    }
    // The schemastery placeholder for "user set nothing".
    if (typeof value === "object" && !Array.isArray(value) && Object.keys(value).length === 0) continue;
    const fallback = defaults[key];
    if (fallback !== undefined) {
      const fallbackIsArray = Array.isArray(fallback);
      if (fallbackIsArray !== Array.isArray(value)) continue;
      if (!fallbackIsArray && typeof fallback !== typeof value) continue;
    }
    merged[key] = value;
  }
  return merged;
...
/**
 * Field contract of the graphical settings page.
 *
 * Only keys listed here **and** marked `.volatile()` appear in
 * 「插件 → 本行 → 配置」 (the host settings mirror projects volatile fields
 * only); everything else stays YAML-only, and unknown keys in the row config
 * are preserved untouched. The schema owns "which keys are editable + their
 * validation + the values the form shows"; the runtime defaults stay owned by
 * {@link DEFAULTS} alone.
 *
 * Loading must be completely defensive. On a harness that cannot resolve
 * `@deepseek-ai/schemastery`, or whose version predates `.volatile()`
 * (the copy shipped with 0.1.5-rc.3 has no such method), this must quietly
 * export no `Config` at all: the plugin keeps working and merely has no
 * settings form. A top-level `import` that throws would fail the whole row and
 * make the plugin disappear from the sidebar — the exact failure class fixed in
 * v0.4.3.
 */
export let Config;
try {
  const { default: Schema } = await import("@deepseek-ai/schemastery");
  /** Bounded integer field, shown with its effective default in the form. */
  const count = (value, min = 0) => Schema.number().min(min).step(1).default(value).volatile();
  Config = Schema.object({
    /** 本地统计缓存与余额缓存的刷新节奏（毫秒）。 */
    localTtlMs: count(DEFAULTS.localTtlMs),
    balanceTtlMs: count(DEFAULTS.balanceTtlMs),
    /** 回放多少个最近会话；越大越准、越慢。 */
    maxSessions: count(DEFAULTS.maxSessions, 1),
    sessionConcurrency: count(DEFAULTS.sessionConcurrency, 1),
    balanceTimeoutMs: count(DEFAULTS.balanceTimeoutMs),
    /**
     * 节假日日历地址；清空即关闭远端、改用内置 2026 表。留空字符串同样视为关闭。
     */
    holidayCalendarUrl: Schema.string().default(DEFAULTS.holidayCalendarUrl).volatile(),
    holidayCalendarTtlMs: count(DEFAULTS.holidayCalendarTtlMs),
    /* eslint-disable-next-line no-restricted-syntax -- 结构化配置由设置页的 JSON 编辑器承载 */
    peakHours: Schema.any().volatile(),
    holidayRanges: Schema.any().volatile(),
    holidayWorkdays: Schema.any().volatile(),
    /** 高级：按模型单价覆盖官方价（元/百万 tokens）。改错会让估算失真。 */
    pricing: Schema.any().volatile(),
    timezoneOffsetMinutes: count(DEFAULTS.timezoneOffsetMinutes)
  });
} catch {
  // No schema: YAML-only configuration, exactly as before.
  Config = undefined;
}
 ...
 * @param cfg - plugin config (newPricingAt, weekendOffPeakAt, holidayRanges, holidayOffPeakAt, peakHours, timezoneOffsetMinutes).
 * @param provider - provider id that served the call (`deepseek-official`, `qwen`, …),
 *   or undefined for logs written before the field existed.
 * @returns `{ input, cacheHit, output }` rates plus `estimated` (rates guessed rather
 *   than looked up) and `billable` (whether this call may be priced at all).
 */
function resolveRates(pricing, model, time, cfg, provider) {
  ...
  // Only calls served by the official DeepSeek API are priced. A locally hosted
  // model (provider `qwen`, `ollama`, …) or a third-party provider costs nothing
  // on the DeepSeek bill, so pricing it at `pricing.default` invents money —
  // that was the "本地部署的模型也被计费" bug.
  //
  // Older logs have no provider: fall back to "the model id is one we know",
  // which still refuses to price unknown ids off the `default` entry.
  const official = typeof provider !== "string" || provider.length === 0 ? null : /deepseek/i.test(provider);
  const billable = official === null ? known : official;
  ...
  if (info.period === "flat" || !isRates(peak) || !isRates(offPeak)) {
    return { ...flat, estimated: !known, billable };
  }
...
function addUsage(bucket, usage, rates) {
  ...
  // Tokens are always counted — only the money is skipped for non-official calls.
  if (rates.billable === false) return;
  ...
  try {
    ...
    for (const event of events) {
      ...
      if (event.type === "request/header" && typeof event.data?.header?.config?.model === "string") {
        ...
        if (typeof event.data.header.config.provider === "string") provider = event.data.header.config.provider;
        continue;
      }
      ...
      // The message's own provenance names the model (and provider) that produced
      // it, which stays right even when the request header events are missing
      // (forked prefixes, rotated logs) or when one session mixed several models.
      const sourceModel = event.data.message?.source?.model;
      if (typeof sourceModel === "string" && sourceModel.length > 0) model = sourceModel;
      const sourceProvider = event.data.message?.source?.provider;
      if (typeof sourceProvider === "string" && sourceProvider.length > 0) provider = sourceProvider;
      const usage = event.data.usage;
      const rates = resolveRates(cfg.pricing, model ?? "unknown", time, cfg, provider);
      const slotKey = model ?? "unknown";
      const slot = byModel[slotKey] ?? (byModel[slotKey] = { model: slotKey, provider, billable: rates.billable !== false, estimated: rates.estimated, ...emptyBucket() });
      slot.billable ||= rates.billable !== false;
      if (slot.provider === undefined && provider !== undefined) slot.provider = provider;
...
async function computeLocalUsage(ctx, cfg) {
  ...
  for (const row of rows) {
    ...
    for (const [modelId, bucket] of Object.entries(row.byModel ?? {})) {
      const target = byModel[modelId] ?? (byModel[modelId] = { model: modelId, provider: bucket.provider, billable: bucket.billable !== false, estimated: false, ...emptyBucket() });
      mergeBucket(target, bucket);
      target.estimated ||= bucket.estimated === true;
      target.billable ||= bucket.billable !== false;
      if (target.provider === undefined && bucket.provider !== undefined) target.provider = bucket.provider;
    }
    delete row.today;
    delete row.week;
    delete row.byDay;
 ...
 */
/**
 * How this copy of the plugin was installed, and how a newer version should be
 * fetched. A `link:` dependency is a development checkout: re-installing it from
 * git would silently replace the developer's working tree with a tagged copy, so
 * that case never gets a one-click update.
 * @returns `{ kind, spec }` — kind is `link` / `git` / `npm` / `unknown`, spec the
 *   pnpm spec for `version` (null when the channel cannot be determined).
 */
function describeInstall(ctx, manifest, version) {
  const fallbackRepo = "https://github.com/xavier711/dsh-deepseek-usage.git";
  const repo = typeof manifest?.repository === "string"
    ? manifest.repository
    : typeof manifest?.repository?.url === "string" ? manifest.repository.url : fallbackRepo;
  const pkgName = typeof manifest?.name === "string" ? manifest.name : "@xavier711/dsh-deepseek-usage";
  const gitSpec = version === null ? null : `git+${repo.replace(/^git\+/, "").replace(/\.git$/, "")}.git#v${version}`;
  const npmSpec = version === null ? null : `${pkgName}@${version}`;
  let declared;
  try {
    const dir = ctx.get?.("profileContext")?.dir;
    if (typeof dir !== "string" || dir.length === 0) return { kind: "unknown", spec: null };
    const profileManifest = JSON.parse(readFileSync(join(dir, "package.json"), "utf8"));
    declared = profileManifest?.dependencies?.[pkgName] ?? profileManifest?.devDependencies?.[pkgName];
  } catch {
    return { kind: "unknown", spec: null };
  }
  if (typeof declared !== "string" || declared.length === 0) return { kind: "unknown", spec: null };
  if (declared.startsWith("link:") || declared.startsWith("file:")) return { kind: "link", spec: null };
  if (declared.includes("github.com") || declared.startsWith("git+")) return { kind: "git", spec: gitSpec };
  return { kind: "npm", spec: npmSpec };
}

function apply(ctx, config = {}) {
  const cfg = mergeConfig(DEFAULTS, unwrapConfig(config ?? {}));
  ...
  async function getUpdate() {
    ...
    try {
      ...
      // Whether the panel may offer a one-click update: the host must expose the
      // plugin manager (older harnesses do not), and this copy must have been
      // installed from git or npm — a `link:` checkout belongs to its author and
      // re-installing it would replace their working tree with a tagged copy.
      const install = describeInstall(ctx, manifest, latest);
      const manager = ctx.get?.("pluginManager");
      const canInstall = typeof manager?.installBundle === "function"
        && (install.kind === "git" || install.kind === "npm")
        && install.spec !== null;
      payload = {
        ok: true,
        checkedAt: Date.now(),
  ...
  const disposers = [
    ...
    ctx.webServer.register({
      kind: "exact",
      path: "/dsh-usage/update",
      handler: async (req, res) => {
        // POST only: an install must never be triggerable by a link prefetch or a
        // stray GET. The spec is always built here from our own manifest + the
        // published version — the client cannot ask us to install something else.
        if (typeof req?.method === "string" && req.method !== "POST") {
          sendJson(res, 405, { ok: false, error: "METHOD_NOT_ALLOWED", message: "POST only" });
          return;
        }
        try {
          const manager = ctx.get?.("pluginManager");
          if (typeof manager?.installBundle !== "function") {
            sendJson(res, 200, { ok: false, reason: "unsupported" });
            return;
          }
          const update = await getUpdate();
          if (update.ok !== true || update.latest === null) {
            sendJson(res, 200, { ok: false, reason: "no-latest", message: update.error ?? null });
            return;
          }
          const install = describeInstall(ctx, JSON.parse(readFileSync(new URL("../package.json", import.meta.url), "utf8")), update.latest);
          if (install.kind === "link") {
            sendJson(res, 200, { ok: false, reason: "linked" });
            return;
          }
          if (install.spec === null) {
            sendJson(res, 200, { ok: false, reason: "unknown-install" });
            return;
          }
          const result = await manager.installBundle(install.spec, {});
          updateCache = null; // the next /version must re-read the fresh manifest
          const failed = result?.application === "failed" || result?.application === "cancelled";
          sendJson(res, 200, {
            ok: !failed,
            reason: failed ? "install-failed" : "installed",
            latest: update.latest,
            spec: install.spec,
            application: result?.application ?? null,
            message: result?.error === undefined ? null : String(result.error)
          });
        } catch (error) {
          sendJson(res, 500, { ok: false, error: "INTERNAL", message: String(error?.message ?? error) });
        }
