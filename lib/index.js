...
/**
 * Epoch ms of V4 Pro's retirement (2026-09-14 12:00 北京时间 → 04:00Z): the
 * model is routed to V4.1 Flash and billed at V4.1 Flash rates from then on.
 */
const V4_PRO_SUNSET_AT = Date.UTC(2026, 8, 14, 4, 0, 0);

/**
 * Rates in force from {@link FLASH_PRICE_CUT_AT} for every flash-series model,
 * and from {@link V4_PRO_SUNSET_AT} for the retired V4 Pro as well. Peak is
 * exactly twice the off-peak rate.
 */
...
const DEFAULTS = {
   ...
   *   - 2026-09-14 12:00（北京时间）起 V4 Pro 下线并路由到 V4.1 Flash，按
   *     V4.1 Flash 计费（即上一条 flash 系列价格），见 `V4_PRO_SUNSET_AT`。
  ...
  pricing: {
    ...
    "deepseek-v4-pro": {
      ...
      // 2026-09-14 12:00 起 V4 Pro 下线，路由到 V4.1 Flash 并按 flash 计费。
      eras: [{ at: V4_PRO_SUNSET_AT, ...FLASH_RATES_FROM_2026_09_10 }]
    },
 ...
 * covering `[0, 1440)`. On Beijing weekends (Sat/Sun, subject to the rule
 * from `weekendOffPeakAt`) the whole day is a single off-peak segment;
 * otherwise the day alternates off-peak/peak around `peakHours`.
 * @param cfg - plugin config (weekendOffPeakAt, peakHours, timezoneOffsetMinutes).
 * @param dayStart - epoch ms of the Beijing midnight starting the day.
 */
function daySegments(cfg, dayStart) {
  if (isBeijingWeekend(cfg, dayStart + cfg.timezoneOffsetMinutes * 60_000)) {
 ...
 * windows entirely: the whole day is one `offPeak` segment. `nextAt` is the
 ...
 * @param cfg - plugin config (newPricingAt, weekendOffPeakAt, peakHours, timezoneOffsetMinutes).
 * @param time - epoch ms of the moment to classify.
...
function beijingPeriod(cfg, time) {
  ...
  // period differs from the current one.
